...
//  final static String DEFAULT_URL = "jdbc:oracle:thin:@localhost:1521:xe";
	final static String DEFAULT_URL = "jdbc:oracle:thin:@192.168.1.28:1521:pmdbajg";
  final static String DEFAULT_USER = "system";
//  final static String DEFAULT_USER = "EDIOptimization";
  final static String DEFAULT_PASSWORD = "p@ssw0rd";
  ...
  private final String TABLE_NAME = "TEST_LONG";
...
//    try (PreparedStatement pstmt = conn.prepareStatement(
//        "INSERT INTO " + TABLE_NAME + " (name, data) VALUES ('vipi2n', ?)")) {
//      // Creates and fill data in the clob
//      Clob clob = conn.createClob();
//      clob.setString(1, "Book Title - JNI for Dumm22ies");
//
//      // Insert clob data in the column of a table.
//      pstmt.setClob(1, clob);
//      pstmt.execute();
...
//    }
  ...
  public void executeClobAsLongQuery(int id) throws Exception {
    try (PreparedStatement pstmt = conn
        .prepareStatement("Select * from ORACLE_TRGT_XMLDB_PARENT")) {
...
//      pstmt.setString(1, "vipin");
      try (ResultSet rset = pstmt.executeQuery()) {
        while (rset.next()) {
          String c = rset.getString(1);
          String c2 = rset.getString(6);

          show("CLOB_DATA as LONG = " + c + c2);
        }
      }catch(Exception e) {
      	e.printStackTrace();
      }
